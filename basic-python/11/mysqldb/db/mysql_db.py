...
class MysqlDB:
    ...
    def __init__(self, arguments):
        self._connection = mysql.connector.connect(arguments)
    ...
    def createTable(self, tableName, primaryKey=str, fields=dict()):
        if self._existTable(tableName) == False:
            ...
            if len(primaryKey) > 0:
                query += f'{primaryKey} serial primary key, '
    ...
    def _getId(self, tableName):
        query = f"""SELECT a.attname FROM pg_index i JOIN pg_attribute a 
                ON a.attrelid = i.indrelid AND a.attnum = ANY(i.indkey) 
                WHERE i.indrelid = '{tableName}'::regclass AND i.indisprimary;"""

        return self._fecthAll(query)[0][0]
